...
class ShoppingCartFeatures {
    public static int counter = 0; // to count the number of calls to print item function
    ...
    public void printingItem(ShoppingCart sc, String finalPrint) throws InterruptedException {
        ...
        if (counter == 1) {
            ...
            sc.printReceipt();
            assertEquals(finalPrint, myOut.toString()); // checking the first case with items first
        } else if (counter == 2) {
            Thread.sleep(1000);
            int i;
            for (i = 0; i < 50; i++) {
                System.out.print("\b"); // removing the previous data printed on console
            }
            final ByteArrayOutputStream myOut = new ByteArrayOutputStream();
            System.setOut(new PrintStream(myOut));

            sc.printReceipt();
            assertEquals(finalPrint, myOut.toString()); // checking the second case with prices first
            counter = 0;
        }
...
// class to implement Single Responsibility Principle
